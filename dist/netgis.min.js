...
netgis.Client.prototype.onContextResponseWMCLegacy=function(a){console.info("CLIENT WMC RESPNSE:",a.output,"->",this.config);var b=this.config&&this.config.wmc?this.config.wmc:null,c=b&&!0===b.layers_removable;(b=a.output.extent)&&this.modules.map.zoomExtent(b[0],b[1],b[2],b[3]);a=a.output.entities;var d={};for(b=0;b<a.length;b++){var e=a[b];if(e.layer&&e.title){for(var f=!1,g=0;g<a.length;g++){var h=a[g],k=h.parent?h.parent:null;if(k&&e===k){f=!0;break}}d[e.layer.id]=f?this.modules.layertree.tree.addFolder(null,
e.layer.id,e.title,!1):this.modules.layertree.addLayerItem({id:e.layer.id,title:e.title,query:!0===e.queryable,removable:c,transparency:e.opacity?1-.01*e.opacity:.2,active:!1},null)}}for(b=0;b<a.length;b++)e=a[b],e.layer&&e.title&&e.parent&&(k=e.parent?e.parent:null)&&!k.service&&this.modules.layertree.tree.setFolderParent(d[e.layer.id],d[k.layer.id]);this.modules.layertree.tree.moveItemDown(this.modules.layertree.tree.getFolder("bg"));this.modules.layertree.tree.moveButtonsDown();for(b=0;b<a.length;b++)e=
a[b],e.layer&&e.kml&&(c={id:e.layer.id.toString(),title:e.title,order:5E4,type:"KML",url:e.kml,opacity:e.opacity?e.opacity:.8,active:!1,query:!0},this.config.layers.push(c));for(b=0;b<a.length;b++)if(e=a[b],e.layer&&e.name){c=[];for(g=0;g<a.length;g++)a[g].parent===e&&c.push(a[g]);f=0<c.length;if(!1===f){h=e;c=e.url?e.url:null;do k=h.parent?h.parent:null,k.service&&(c=k.url),h=k;while(!c&&k);c={id:e.layer.id.toString(),title:e.title,order:a.length-b,type:"WMS",url:c,name:e.name,opacity:e.opacity?
e.opacity:.8,active:!1,query:!0===e.queryable};this.config.layers.push(c)}}for(b=0;b<a.length;b++)e=a[b],e.layer&&e.active&&netgis.util.invoke(this.container,netgis.Events.MAP_LAYER_TOGGLE,{id:e.layer.id.toString(),on:e.active});console.info("CONTEXT CONFIG:",this.config);this.showLoader(!1)};
...
netgis.Import.prototype.updateGeoportalResults=function(a,b){this.geoportalData={data:a,folders:{}};a=a.wms.srv;if(b){b={};for(var c=0;c<a.length;c++){var d=a[c],e=d.title;d.wmsRootLayerId&&(e=d.wmsRootLayerId);if(b[e]){var f=b[e];f.getElementsByTagName("summary")}else f=this.geoportalResults.addFolder(null,e,d.title,!1,!1,!1,!1,this.config["import"].clip_titles),f.setAttribute("title",d["abstract"]),f.setAttribute("data-url",d.wmsGetCapabilitiesUrl),f.setAttribute("data-title",d.title),f.getElementsByTagName("details")[0].addEventListener("toggle",
...
m.setAttribute("title",n.abstract),m.setAttribute("data-title",n.title),d&&this.createGeoportalLayers(m,b,n.children,!0))}};netgis.Import.prototype.onGeoportalSearchButtonClick=function(a){this.geoportalSearch.onInputTimeout()};
...
netgis.WMCLegacy.prototype.onContextResponse=function(a){if(netgis.util.isJSON(a)){a=JSON.parse(a);console.info("WMC RESPONSE:",a);var b=a.wmc.bbox;if(b){b=b.split(",");for(var c=0;c<b.length;c++)b[c]=parseFloat(b[c]);!a.wmc.crs||"string"===typeof a.wmc.crs||a.wmc.crs instanceof String||(a.wmc.crs=a.wmc.crs[0]);if(a.wmc.crs&&a.wmc.crs!==this.config.map.projection){c=proj4(a.wmc.crs,this.config.map.projection,[b[0],b[1]]);var d=proj4(a.wmc.crs,this.config.map.projection,[b[2],b[3]]);b[0]=c[0];b[1]=
c[1];b[2]=d[0];b[3]=d[1]}this.output.extent=[b[0],b[1],b[2],b[3]]}(b=a.wmc.kmloverlay)&&0<b.length&&this.output.entities.push({layer:{id:"kml"},title:"KML",kml:b,active:!0});b=[];for(c=0;c<a.layerList.length;c++)d=a.layerList[c],b.push(d.layerId),this.output.entities.push({layer:{id:Number.parseInt(d.layerId)},position:d.layerPos,active:d.active,opacity:d.opacity?.01*Number.parseFloat(d.opacity):1});this.requestLayers(b)}else console.error("could not parse WMC response:",a),this.callback&&this.callback({config:null,
output:{}})};netgis.WMCLegacy.prototype.requestLayers=function(a){console.info("WMC REQUEST LAYERS:",a);var b=this.config.wmc;b?(b=b.layers_url,b=netgis.util.replace(b,"{ids}",a.join(",")),netgis.util.request(b,this.onLayersResponse.bind(this))):console.error("no config[ 'wmc' ] found, skipping WMC layer loading")};
