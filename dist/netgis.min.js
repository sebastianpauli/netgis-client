...
netgis.Client.prototype.onContextResponseLayer=function(a){var b=JSON.parse(a);a=new netgis.WMC;var c=b.wms.srv;c&&0!==c.length?(b=c[0],a=a.parseServiceLayer(b.id.toString(),b,null,b.layer[0],null),this.config.layers.push(a),netgis.util.invoke(this.container,netgis.Events.MAP_LAYER_CREATE,a),a.bbox&&netgis.util.invoke(this.container,netgis.Events.MAP_ZOOM_BBOX,{minlon:a.bbox[0],minlat:a.bbox[1],maxlon:a.bbox[2],maxlat:a.bbox[3]})):console.error("no wmc service layer in context response",b)};
netgis.Client.prototype.requestContextOWS=function(a){console.info("Request OWS:",a);netgis.util.request(a,this.onContextResponseOWS.bind(this))};netgis.Client.prototype.onContextResponseOWS=function(a){a=JSON.parse(a);console.info("OWS Response:",a);a=netgis.OWS.read(a,this);console.info("OWS Config:",a)};netgis.Client.prototype.onMapEditLayerChange=function(a){a=JSON.stringify(a.detail.geojson);this.output.value=a};
...
netgis.Import.prototype.updateGeoportalResults=function(a,b){this.geoportalData={data:a,folders:{}};console.info("GEOPORTAL DATA:",this.geoportalData);a=a.wms.srv;if(b){b={};for(var c=0;c<a.length;c++){var d=a[c],e=d.title;d.wmsRootLayerId&&(e=d.wmsRootLayerId);if(b[e]){var f=b[e];f.getElementsByTagName("summary")}else f=this.geoportalResults.addFolder(null,e,d.title,!1,!1,!1,!1,this.config["import"].clip_titles),f.setAttribute("title",d["abstract"]),f.setAttribute("data-url",d.wmsGetCapabilitiesUrl),
f.setAttribute("data-title",d.title),f.setAttribute("data-root-id",d.wmsRootLayerId),f.getElementsByTagName("details")[0].addEventListener("toggle",this.onGeoportalFolderToggle.bind(this)),f.getElementsByTagName("input")[0].setAttribute("disabled","disabled"),b[e]=f}this.geoportalData.folders=b}this.geoportalResults.container.scrollTop=0;this.geoportalLoader.classList.add("netgis-hide")};
...
netgis.Import.prototype.onGeoportalFolderResponse=function(a,b){a.getElementsByTagName("ul")[0].innerHTML="";a.getElementsByTagName("input")[0].removeAttribute("disabled");b=netgis.WMS.parseCapabilities(b);var c=!0;this.config["import"]&&!1===this.config["import"].geoportal_order_reverse&&(c=!1);this.createGeoportalLayers(a,b,b.layers,!0,c);this.config["import"]&&!0===this.config["import"].geoportal_simplify_folders&&this.simplifyGeoportalFolder(a);b=!1;this.config["import"]&&!0===this.config["import"].geoportal_order_by_meta&&
(b=!0);if(b){b=[];var d=this.geoportalData.data.wms.srv;for(c=0;c<d.length;c++){b.push(d[c].title);var e=d[c].layer;if(e)for(var f=0;f<e.length;f++){b.push(e[f].title);var g=e[f].layer;if(g)for(var h=0;h<g.length;h++)b.push(g[h].title)}}d=a.getElementsByTagName("li");for(c=0;c<d.length;c++)e=d[c],f=b.indexOf(e.innerText),e.setAttribute("data-order",f);this.geoportalResults.reorderByAttribute("data-order",!1,!0,a.getElementsByTagName("ul")[0])}};
...
netgis.Import.prototype.createGeoportalLayers=function(a,b,c,d,e){var f=a.getAttribute("data-id"),g=b.requests.map.url,h=b.requests.info.url,k=null;-1<b.requests.map.format.indexOf("image/jpeg")&&(k="image/jpeg");-1<b.requests.map.format.indexOf("image/png")&&(k="image/png");var l=null;-1<b.requests.info.format.indexOf("text/plain")&&(l="text/plain");-1<b.requests.info.format.indexOf("text/html")&&(l="text/html");for(var n=0;n<c.length;n++){var m=c[n],p=f+"_"+m.name;m.id=p;0===m.children.length?(p=
this.geoportalResults.addCheckbox(a,p,m.title,!1,e,null,this.config["import"].clip_titles),p.setAttribute("title",m.abstract),p=p.getElementsByTagName("input")[0],p.setAttribute("data-map-url",g),p.setAttribute("data-info-url",h),p.setAttribute("data-map-format",k),p.setAttribute("data-info-format",l),p.setAttribute("data-name",m.name),p.setAttribute("data-title",m.title),p.setAttribute("data-queryable",m.queryable)):(p=this.geoportalResults.addFolder(a,p,m.title,e,!1,!1,!1,this.config["import"].clip_titles),
p.setAttribute("title",m.abstract),p.setAttribute("data-title",m.title),d&&this.createGeoportalLayers(p,b,m.children,!0,e))}};
...
a.layers)for(g=0;g<a.length;g++){b=a[g];h=f[b.folder]?f[b.folder]:null;k=b.id?b.id:g.toString();var l=!1;if(h)for(var n=0;n<e.length;n++){var m=e[n];m.id===b.folder&&(l=m.radio)}n=b.title;var p='<i class="fas fa-mouse-pointer" title="Ebene ist abfragbar"></i>';m=!1;d&&d.query_icon&&(p=d.query_icon);(!0===b.query||b.query_url&&""!==b.query_url)&&p&&""!==p&&(m=!0);p=b.removable;k=!0===l?this.tree.addRadioButton(h,k,n,b.active,this.createDefaultDetails(b,!0,p,m),d?d.clip_titles:!1):this.tree.addCheckbox(h,
k,n,b.active,!1,this.createDefaultDetails(b,!0,p,m),d?d.clip_titles:!1);k.addEventListener("contextmenu",this.onTreeItemMenu.bind(this));b.data&&this.tree.setItemData(k,b.data);!0===b.hidden&&k.classList.add("netgis-hide")}this.tree.updateFolderChecks();if(!0===c){if(d&&d.buttons)for(c=d.buttons,g=0;g<c.length;g++)e=c[g],this.tree.addButton(null,e.id,e.title,this.onTreeButtonClick.bind(this));d&&!0===d.open&&this.panel.show()}};
...
g[0].values;for(var h=0;h<f.length;h++){g=d.exec("SELECT * FROM "+f[h][0]);var k=g[0];g=null;for(var l=0;l<k.columns.length;l++){if("geometry"===k.columns[l].toLowerCase()){g=l;break}if("geom"===k.columns[l].toLowerCase()){g=l;break}}if(null!==g)for(k=k.values,l=0;l<k.length;l++){var n=k[l][g],m=new Uint8Array(n.length-43-1+5);m[0]=n[1];m[1]=n[39];m[2]=n[40];m[3]=n[41];m[4]=n[42];for(var p=n.length-43-1,q=0;q<p;q++)m[5+q]=n[43+q];n=(new ol.format.WKB).readGeometry(m,{featureProjection:c.view.getProjection()});
...
c;var p=m.getContext("2d");p.webkitImageSmoothingEnabled=!1;p.mozImageSmoothingEnabled=!1;p.imageSmoothingEnabled=!1;Array.prototype.forEach.call(document.querySelectorAll(".ol-layer canvas"),function(a){if(0<a.width){var b=a.parentNode.style.opacity;p.globalAlpha=""===b?1:Number(b);b=a.style.transform.match(/^matrix\(([^\(]*)\)$/)[1].split(",").map(Number);CanvasRenderingContext2D.prototype.setTransform.apply(p,b);p.drawImage(a,0,0)}});p.drawImage(l,0,0);p.fillStyle="#fff";p.fillRect(0,m.height-
30,140,30);p.fillStyle="#000";p.font="4mm sans-serif";p.fillText(netgis.util.getTimeStamp(),10,m.height-10);var q=document.createElement("a");switch(a){case "pdf":e=e?e:0;var r=297-e-e,v=210-e-e,t=m.width/m.height;if(!d){var w=r;r=v;v=w}if(m.height>m.width){var u=v;w=u*t;w>r&&(w=r,u=w/t)}else w=r,u=w/t,u>v&&(u=v,w=u*t);t=new jsPDF(d?"l":"p");var x=e;x+=(r-w)/2;r=e;r+=(v-u)/2;t.addImage(m.toDataURL("image/png,1.0",1),"PNG",x,r,w,u);t.setFillColor(255,255,255);t.rect(x,r+u-11,80,11,"F");t.setFontSize(8);
t.text("Datum: "+netgis.util.getTimeStamp(),x+2,r+u-2-4);t.text("Quelle: "+window.location.href,x+2,r+u-2);m=t.output("bloburl",{filename:k.default_filename+".pdf"});window.open(m,"_blank");break;case "jpeg":window.navigator.msSaveBlob?window.navigator.msSaveBlob(m.msToBlob(),k.default_filename+".jpg"):(q.setAttribute("download",k.default_filename+".jpg"),q.setAttribute("href",m.toDataURL("image/jpeg",1)),q.click());break;case "png":window.navigator.msSaveBlob?window.navigator.msSaveBlob(m.msToBlob(),
k.default_filename+".png"):(q.setAttribute("download",k.default_filename+".png"),q.setAttribute("href",m.toDataURL("image/png",1)),q.click());break;case "gif":q.setAttribute("download",k.default_filename+".gif"),v=new GIF({workerScript:k.gif_worker,quality:1}),v.addFrame(m),v.on("finished",function(a){q.setAttribute("href",window.URL.createObjectURL(a));q.click()}),v.render()}h.setTarget(g);g.removeChild(n);netgis.util.invoke(f.container,netgis.Events.EXPORT_END,null)});h.renderSync()};l.src=k.logo};
...
l.id);for(g=0;g<f.length;g++)h.push(f[g]);for(g=0;g<h.length;g++)if(l=h[g],k=this.getLayerMetaData(l.id))if(n=k.layer)for(k=n.length-1;0<=k;k--){if(m=n[k],!(m.layer&&0<m.layer.length)){var p=this.getLayerExportData(m.id);if(f=m.bbox){f=f.split(",");for(var q=0;q<f.length;q++)f[q]=Number.parseFloat(f[q])}f={folder:l.id,id:m.id,title:m.title,active:p.active,query:1===p.layerQueryable,transparency:1-.01*p.opacity,type:netgis.LayerTypes.WMS,url:this.getLayerMapURL(m.id),name:m.name,format:"image/png",
bbox:f,removable:d,data:{order:this.getLayerExportData(m.id).layerPos}};a.push(f)}}else console.error("layer folder without children",l.id);else console.error("no metadata found for layer folder",l.id);var r=this;a.sort(function(a,b){b=b.id;a=r.getLayerExportData(a.id);b=r.getLayerExportData(b);a=a.layerPos;b=b.layerPos;if(c.layers_inverse){if(a<b)return 1;if(a>b)return-1}else{if(a<b)return-1;if(a>b)return 1}return 0});d=1E3+b.length;for(g=0;g<b.length;g++)for(h=b[g].layerId.toString(),k=0;k<a.length;k++)f=
...
b))?a.innerHTML:null},buildRequestURL:function(a,b){var c=netgis.util.parseURL(a,!0);a=c.base;c=c.parameters;for(var d in c)"service"===d.toLowerCase()&&delete c[d],"request"===d.toLowerCase()&&delete c[d];console.info("CLEAN PARAMS:",c);c.service="WMS";c.request=b;b=[];for(d in c)b.push(d+"="+c[d]);return a+"?"+b.join("&")}};
netgis.build="20260916"; 
