...
netgis.Client.prototype.initConfig=function(a){a&&a.wmc&&a.wmc.url&&a.wmc.id&&this.requestContextWMC(a.wmc.url,a.wmc.id);a&&a.ows&&a.ows.url&&this.requestContextOWS(a.ows.url)};
...
netgis.LayerTree.prototype.initConfig=function(a,b,c){var d=this.config.layertree;d&&d.title&&this.panel.setTitle(d.title);var e=a.folders,f={};if(e){for(var g=0;g<e.length;g++){var h=e[g],k=this.tree.addFolder(null,h.id,h.title,b,!1,h.draggable,h.removable,d?d.clip_titles:!1);f[h.id]=k;!0===h.open&&this.tree.setFolderOpen(h.id,!0)}for(g=0;g<e.length;g++)h=e[g],k=h.id,h=h.parent,-1===h&&(h=null),""===h&&(h=null),h&&this.tree.setFolderParent(f[k],f[h])}if(a=a.layers)for(g=0;g<a.length;g++){b=a[g];
h=f[b.folder]?f[b.folder]:null;k=b.id?b.id:g.toString();var l=!1;if(h)for(var n=0;n<e.length;n++){var m=e[n];m.id===b.folder&&(l=m.radio)}n=b.title;m='<i class="fas fa-mouse-pointer" title="Ebene ist abfragbar"></i>';d&&d.query_icon&&(m=d.query_icon);(!0===b.query||b.query_url&&""!==b.query_url)&&m&&""!==m&&(n+='<span class="netgis-right">'+m+"</span>");m=b.removable;k=!0===l?this.tree.addRadioButton(h,k,n,b.active,this.createDefaultDetails(b,!0,m),d?d.clip_titles:!1):this.tree.addCheckbox(h,k,n,
b.active,!1,this.createDefaultDetails(b,!0,m),d?d.clip_titles:!1);k.addEventListener("contextmenu",this.onTreeItemMenu.bind(this));!0===b.hidden&&k.classList.add("netgis-hide")}this.tree.updateFolderChecks();if(!0===c){if(d&&d.buttons)for(c=d.buttons,g=0;g<c.length;g++)e=c[g],this.tree.addButton(null,e.id,e.title,this.onTreeButtonClick.bind(this));d&&!0===d.open&&this.panel.show()}};
...
netgis.LayerTree.prototype.addLayerItem=function(a,b){var c=a.title,d='<i class="fas fa-mouse-pointer" title="Ebene ist abfragbar"></i>';this.config.layertree&&this.config.layertree.query_icon&&(d=this.config.layertree.query_icon);(!0===a.query||a.query_url&&""!==a.query_url)&&d&&""!==d&&(c+='<span class="netgis-right">'+d+"</span>");this.tree.addCheckbox(b,a.id,c,!0,!0,this.createDefaultDetails(a,!0,!0),this.config.layertree.clip_titles).addEventListener("contextmenu",this.onTreeItemMenu.bind(this))};
...
{parent:null,id:f.layerId,title:f.layerId,removable:d},l=this.getLayerMetaData(f.layerId);l&&(k.title=l.title);h.push(k)}f=[];for(g=0;g<h.length;g++){k=h[g];l=this.getLayerMetaData(k.id);var n=l.layer;for(l=n.length-1;0<=l;l--){var m=n[l];m.layer&&0!==m.layer.length&&f.push({parent:k.id,id:m.id,title:m.title,removable:d})}}for(g=0;g<f.length;g++)h.push(f[g]);for(g=0;g<h.length;g++)for(k=h[g],l=this.getLayerMetaData(k.id),n=l.layer,l=n.length-1;0<=l;l--)if(m=n[l],!(m.layer&&0<m.layer.length)){var q=
this.getLayerExportData(m.id);if(f=m.bbox){f=f.split(",");for(var p=0;p<f.length;p++)f[p]=Number.parseFloat(f[p])}f={folder:k.id,id:m.id,title:m.title,active:q.active,query:1===q.layerQueryable,transparency:1-.01*q.opacity,type:netgis.LayerTypes.WMS,url:this.getLayerMapURL(m.id),name:m.name,format:"image/png",bbox:f,removable:d};a.push(f)}var r=this;a.sort(function(a,b){b=b.id;a=r.getLayerExportData(a.id);b=r.getLayerExportData(b);a=a.layerPos;b=b.layerPos;if(c.layers_inverse){if(a<b)return 1;if(a>
b)return-1}else{if(a<b)return-1;if(a>b)return 1}return 0});d=1E3+b.length;for(g=0;g<b.length;g++)for(h=b[g].layerId.toString(),l=0;l<a.length;l++)f=a[l],f.id===h&&(f.order=d-g);return e};netgis.WMC.prototype.getLayerExportData=function(a){for(var b=this.exportData.layerList,c=0;c<b.length;c++){var d=b[c];if(d.layerId.toString()===a)return d}return null};
netgis.WMC.prototype.getLayerMetaData=function(a,b){b||(b=this.metaData.wms.srv);for(var c=0;c<b.length;c++){var d=b[c];if(d.layer){var e=this.getLayerMetaData(a,d.layer);if(e)return e}if(d.id===a)return d}return null};netgis.WMC.prototype.getLayerMapURL=function(a,b,c){b||(b=this.metaData.wms.srv);for(var d=0;d<b.length;d++){var e=b[d];e.getMapUrl&&""!==e.getMapUrl&&(c=e.getMapUrl);if(e.layer){var f=this.getLayerMapURL(a,e.layer,c);if(f)return f}if(e.id===a)return c}return null};netgis=netgis||{};
