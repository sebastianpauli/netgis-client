...
netgis.Import.prototype.initSections=function(a){this.sections={};var b=0;if(a["import"]&&!0===a["import"].geoportal_tab){this.sections.geoportal=this.tabs.getContentSection(b);b+=1;this.sections.geoportal.classList.add("netgis-geoportal");this.geoportalSearch=new netgis.Search("Thema, Schlagwort...");this.geoportalSearch.autocomplete=a["import"].geoportal_autocomplete;this.geoportalSearch.container.addEventListener(netgis.Events.SEARCH_CHANGE,this.onGeoportalSearchChange.bind(this));this.geoportalSearch.container.addEventListener(netgis.Events.SEARCH_CLEAR,
...
netgis.WMC.prototype.onContextResponse=function(a){if(netgis.util.isJSON(a)){var b=JSON.parse(a);this.data=b;a=[];if(b=b.layerList)for(var c=0;c<b.length;c++)a.push(b[c].layerId);this.requestLayers(a)}else console.error("could not parse WMC response:",a),this.callback&&this.callback({config:null})};netgis.WMC.prototype.requestLayers=function(a){var b=this.config.wmc;b?(b=b.layers_url,b=netgis.util.replace(b,"{ids}",a.join(",")),netgis.util.request(b,this.onLayersResponse.bind(this))):console.error("no config[ 'wmc' ] found, skipping WMC layer loading")};
netgis.WMC.prototype.onLayersResponse=function(a){this.layers=a=JSON.parse(a);console.info("WMC Layers Response:",a);this.callback&&this.callback({config:this.toConfig()})};
...
q=null;if(b)for(var p=0;p<b.length;p++)if(b[p].layerId.toString()===m){q=b[p];break}n=this.parseServiceLayer(m,f,h.id,n,q);d.push(n)}}}return{folders:c,layers:d}};
netgis.WMC.prototype.parseServiceLayer=function(a,b,c,d,e){return{id:a,folder:c,title:d.title,active:e?e.active:!0,query:1===d.queryable,transparency:e?1-.01*e.opacity:0,order:1E3,type:netgis.LayerTypes.WMS,url:b.getMapUrl,name:d.name,format:e?e.currentFormat:"image/png",removable:this.config.wmc&&!0===this.config.wmc.layers_removable?!0:!1}};
